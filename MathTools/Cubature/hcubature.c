...
static rule *make_rule75genzmalik(unsigned dim, unsigned fdim)
{
	...
	slow and is superseded by other methods (e.g. Monte-Carlo). */
     ...
     r->weight1 = (realb(12824 - 9120 * to_int(dim) + 400 * isqr(to_int(dim)))
		   / realb(19683));
     r->weight3 = realb(1820 - 400 * to_int(dim)) / realb(19683);
     r->weight5 = realb(6859) / realb(19683) / realb(1U << dim);
     r->weightE1 = (realb(729 - 950 * to_int(dim) + 50 * isqr(to_int(dim)))
		    / realb(729));
     r->weightE3 = realb(265 - 100 * to_int(dim)) / realb(1458);
