...
struct Func{
    ...
    long double df(const long double x){
        return 2.0*x+6;
    }
};
...
struct Dbrent {
    ...
    template <class T>
    long double minimize(const long double ax,const long double bx,const long double cx,T &funcd)
    {
        const int ITMAX=100;
...
Func y;
Dbrent z;
long double a=53.0, b=2.0;


cout << endl<< "Això minimitza la funcio indicada al codi. El minim es:" << endl << "----------------------------------" <<  endl << endl;


cout << "El minim es " << "\t" << z.minimize(-100,50,100,y) << endl;
//cout << a << "  " << b << endl;
cout << endl << "----------------------------------" << endl;
